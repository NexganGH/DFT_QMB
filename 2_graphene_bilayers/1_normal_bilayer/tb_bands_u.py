...
class TBBandManagerU:
    ...
    def __init__(self, atoms):
        ...
        # --- Hartree–Fock (mean-field) parameters ---
        self.U = 0.0  # on-site Hubbard U (set >0 to enable)
        # mean occupations: (A1, B1, A2, B2)
        self.n_mf = np.zeros(4)

    def H_hartree(self):
        """
        On-site Hartree mean-field contribution:
        diag(U * n_i)
        """
        return np.diag(self.U * self.n_mf)
    ...
    def H_bilayer(self, kx, ky, params):
        ...
        # --- Hartree mean-field interaction ---
        if self.U != 0.0:
            H = H + self.H_hartree()

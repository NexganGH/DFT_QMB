...
# ============================================================
# Polarisation + gap (absolute TB proxy)
# NOTE: sign convention FIXED here: P = (layer1 - layer2)
# ============================================================
...
# ============================================================
# Self-consistency with consistent polarisation shift
#   P_phys(Delta) = P_abs(Delta) - P_abs(b0)
#   Delta = Delta_ext + b0 + U_H * P_phys(Delta)
# ============================================================
DELTA_EXT_SIGN = -1.0  # try -1 if DFT P sign is opposite
def Delta_ext(Ez):
...
def main():
    ...
    # Reference shift: P_ref = P_abs(b0)
    P_ref, Eg_ref = tb_Pabs_and_gap(b0_eV, kpatch)
    print("[REF] Using P_phys(Delta) = P_abs(Delta) - P_abs(b0).")
    print(f"[REF] b0 = {b0_eV:.6f} eV")
    print(f"[REF] P_ref = {P_ref:.6e} e/cell (absolute TB)")
    ...
    plt.xlabel("Ez (V/Å)")
    plt.ylabel("P (e/cell)")
    plt.title("Polarisation vs field (shifted, sign-matched)")
    plt.legend()
    plt.tight_layout()

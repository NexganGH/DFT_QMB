...
####LINE 96 PUT THE NUMBER OF STRANDS Ny YOU HAVE TO COSNIDER
...
####LINE 105 PUT THE NUMBER OF STRANDS Ny YOU HAVE TO COSNIDER
####LINE 213 PUT THE RIGHT NAME OF THE FILE FOR THE MAGNETISATION CORRECTING THE RIGHT NUMBER OF STRANDS
...
# file produced by your new smoothing script
dft_file = "zgnr_Ny1_M1_pi_two_above_from_Z.npz"
data_dft = np.load(dft_file)

k_dft = data_dft["k_dimless"]    # (Nk,)

# In that file the two central bands are stored as E1 and E2
# We just call them "val" and "cond" for the fit, even if
# both may be above EF in the Ny=1 case.
E_val_dft  = data_dft["E1"]      # (Nk,)
E_cond_dft = data_dft["E2"]      # (Nk,)
...
mag_dft_file = "zgnr_Ny1_M1_mag_AB_strands.npz"
...
# ============================================================
# 7. Save fitted parameters and data for post-processing
# ============================================================

# recompute RMS error at the best-fit (same definition as in objective)
err_val = E_val_tb - E_val_dft
err_cond = E_cond_tb - E_cond_dft
err_all = np.concatenate([err_val, err_cond])
rms_bands = np.sqrt(np.mean(err_all**2))

out_file = f"zgnr_Ny{Ny}_M1_tb_fit.npz"
...
print(f"Saved fit data to '{out_file}'")
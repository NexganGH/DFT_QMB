...
def main():
    ...
    # --- ZGNR-4 fit file ---
    tb_npz = os.path.join(base, "zgnr4_nonmag_tb_fit_central_only.npz")
    # or:
    # tb_npz = os.path.join(base, "zgnr4_nonmag_tb_fit_full_pi.npz")

    if not os.path.exists(tb_npz):
        raise FileNotFoundError(f"Missing TB fit file: {tb_npz}")

    out_png = os.path.join(base, "zgnr4_central_pi_fit_DFT_vs_TB.png")
    ...
    E_pi_dft = np.array(data["E_pi_dft"], float)
    E_tb = np.array(data["E_tb"], float)
    ...
    dft2 = pick_two_from_matrix(E_pi_dft, central_idx, "E_pi_dft")
    tb2  = pick_two_from_matrix(E_tb,      central_idx, "E_tb")
